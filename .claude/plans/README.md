...
| 21 | [Retire the camp-report page type](21-retire-camp-reports.md) — camps become newsletter issues, full stop: delete `CampReportPage`/`CampReportIndexPage`, the `/reports/` camp teaser and the seeding, after first converting the one remaining camp report (the **Inauguration Report**, still live — only the Sugar Camp was converted in Plan 20) into a `NewsletterPage`. Redirects preserve both retired URLs. Main risks: deleting a Wagtail page type is a treebeard-aware data migration, and the consent-gate test coverage must not be lost with the type | 📝 Drafted |
