...
| 18a | [Free-text summary privacy remediation](18-notes-privacy-remediation.md) — stakeholder feedback (2026-07-25) surfaced that the Plan 15 N=3 k-anonymity floor runs at publish time only and deployed *after* every live report was published, so 14 sub-floor summaries (10 of them describing a single patient) were public. Adds a `scrub_subfloor_freetext_summaries` backfill command, `noindex` on daily report pages, and a `robots.txt`. Floor stays at 3 (D1, maintainer decision). Merged (PR #144) and deployed; the scrub ran in production on 2026-07-25, blanking all 14 sub-floor summaries across 11 pages (11 revisions scrubbed) with a re-run confirming 0 remaining. Outstanding: a Google Search Console removal request for any of those URLs already indexed | ✅ Done |
| 18b | [Mobile menu + dashboard responsive revision](18b-mobile-menu-and-dashboard-responsive.md) — Track A: the nav drawer's current-page marker (new `--color-nav-current` token, invisible in dark mode before), Donate moved out of the page list into its own CTA block, and the Donate label back to dark ink on amber. Track B: the clinic dashboard's revised range controls (preset tile grid + date card, approved mobile option 1c) and its full-width revenue layout, replacing the page's media queries with auto-fitting grids. Both from one updated design handoff: [`clinic-dashboard-handoff.md`](../../docs/design/clinic-dashboard-handoff.md) (revised) and [`mobile-menu-handoff.md`](../../docs/design/mobile-menu-handoff.md) (new) | ✅ Done |
| 19 | [Upcoming events card (The Family Network feedback, July 2026)](19-family-network-feedback-2026-07.md) — home-page "upcoming events" hero corner card (design handoff option 1b, approved), backed by a new `UpcomingEvent` Wagtail snippet. The rest of that WhatsApp feedback round (About/founders teaser, expandable Contact/Zakat, housekeeping) is already done/merged and no longer tracked here. Design + implementation shipped in `feat/home-upcoming-events`; only the maintainer's own step (entering the real 2026-08-06 camp in `/admin/`) is outstanding | 🚧 In progress |
| 20 | [Sugar Camp report + July newsletter](20-sugar-camp-report-and-july-newsletter.md) — **Track A shipped:** the 6 Aug 2026 Free Sugar Camp is live at `/en/newsletters/free-sugar-camp/` in the Beacon format, with both photos as consent-confirmed "In focus" splits. It began as a `CampReportPage` and moved because that type structurally can't do stat bands or splits (the camp report is unpublished not deleted; its old URL 301s across). All 93 attendees' figures were recomputed from the export in memory — aggregates only, nothing persisted. **Track B decided:** the July issue is a **graphic only**, so no page was created; the plan carries the corrected spec — children/adults **68/99** (not 69/98), the four real age bands replacing a fabricated five-bucket donut, and 6.7 per clinic day. The earlier "staff portraits look AI-generated" finding is **withdrawn — they are real photographs of real staff.** **Track C resolved:** camps stay out of the pipeline, so 6 Aug correctly reads 16 visits and `/reports/` now states that totals exclude camps. The one outstanding item is the graphic redraw, which is design work handed off rather than repo work; the follow-on lives in [Plan 21](21-retire-camp-reports.md) | ✅ Done |
| 21 | [Retire the camp-report page type](21-retire-camp-reports.md) — camps are newsletter issues now, full stop: `CampReportPage`/`CampReportIndexPage`, their templates, factories, seeding and the `/reports/` camp teaser (plus `ReportIndexPage.camp_reports_intro`) are all gone. **No conversion was needed in the end** — the plan assumed the Inauguration Report had to become a new `NewsletterPage`, but the existing "A new chapter begins" (page 52) already *was* it, so the short-lived new page was deleted again and its PDF linked into page 52's body instead; the plan's D1/D2 and its correction table are updated to match. Both retired page URLs already 301'd; the archive index `/en/camp-reports/` needed a **post-deploy redirect content op**, recorded in the plan's release section. **D5's migration order had to be reversed** — schema first, data second: deleting page rows first fails on the FK once the model classes are gone, and deleting the concrete rows through a historical MTI model silently cascades upward and corrupts `numchild`. Both failure modes were caught on a local replica of production; the shipped pair verifies clean against `Page.find_problems()`, no-ops on a fresh install, and refuses to run if a `CASCADE` redirect would be collateral. Consent-gate coverage confirmed by breaking the gate and watching the two surviving guards go red | ✅ Done |
| 22 | [Revenue ingest (Plan 16 Phase 2)](22-revenue-ingest.md) — the clinic software's fee columns shipped, so ingest them and light up the revenue surfaces Plan 16 Phase 1 already built behind `has_revenue`. Six new export columns confirmed header-row-only (`Registration`/`Consultation`/`Lab`/`Ultrasound`/`Pharmacy Fee (PKR)` + `Total Paid (PKR)`), matching the design handoff's placeholder service list exactly. Revenue is stored per visit so `recompute_daily_aggregates` stays non-destructive (D2), keeps a third `unknown` payment bucket so no money vanishes (D3), and treats `Total Paid` as a reconciliation check that warns rather than blocks (D5) — the 6 Aug camp's fabricated PKR 20 fee is why. The new per-row `Date` column and a live home-band `zakat_avg_spend` are both deliberately parked (D6/D8). **Verified against the real 6 Aug export through the real upload view:** every row there has a blank `Status`, so all 93 patients and all PKR 1,860 land in `unknown` — a two-bucket design would have silently dropped the day's entire revenue. The D5 reconciliation correctly stayed silent on that file because it is internally consistent (five fee columns and `Total Paid` both sum to 1,860) and externally wrong, which is the limit of what any code guard can catch here. Tasks 22.1–22.3 merged (PR #162); 22.4 parked | ✅ Done |

| 23 | [Let Neon scale to zero](23-healthz-scale-to-zero.md) — Neon warned at 80% of the free tier's 100 CU-hour monthly compute allowance on a site measuring 15 page views/day. Cause: `/healthz` ran a `SELECT 1` and Render polls `healthCheckPath` roughly **every 5 seconds**, so the compute never got the 5 consecutive idle minutes scale-to-zero needs — 18 days continuously active, ~195 CU-hr/month projected. Measured, not assumed: four `pg_stat_activity` samples put the last `SELECT 1` 3.6–4.0 s in the past, on a 5-second grid phase-locked to `.84x`. The documented 60 s UptimeRobot poll was **not** the culprit and pausing it would have fixed nothing. Splits liveness (`/healthz`, no DB, guarded by `assertNumQueries(0)` over the whole request — middleware could re-arm it) from readiness (`/readyz`, keeps the DB check, called once per release by `scripts/release.sh`). Deliberately gives up between-deploy "app up, DB down" alerting (D4 — noting that check didn't actually work in the 2026-07-26 outage it was written for), and raises `DB_CONNECT_TIMEOUT` to 15 because cold resumes become real for the first time (D5) | 🚧 In progress |
