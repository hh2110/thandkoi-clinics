...
| 20 | [Sugar Camp report + July newsletter](20-sugar-camp-report-and-july-newsletter.md) — Track A: the 6 Aug 2026 Free Sugar Camp as a `CampReportPage` (content only, no PR — the Inauguration Report is the template), with 93 attendees' figures verified from the export in memory. Track B: the July 2026 Beacon issue planned against the real `DailyAggregate` figures — the proposed design's headline numbers check out, but its age-group donut is fabricated (the pipeline has only four bands) and its staff portraits appear AI-generated. Track C: the camp's 93 attendees are absent from the pipeline, and ingest's wholesale date-supersede means uploading the export would *delete* that day's 16 OPD visits rather than add to them — needs a maintainer decision before any upload. **Track A published 2026-08-10**, in the newsletter/Beacon format at `/en/newsletters/free-sugar-camp/` — the maintainer asked for the newsletter's look, which `CampReportPage` structurally can't do, so it shipped as a `NewsletterPage` (camp report unpublished not deleted, old URL 301s across). Photos added (consent confirmed). Tracks B and C outstanding | 🚧 In progress |
