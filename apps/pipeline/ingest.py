...
def recompute_daily_aggregate(
    clinic_date: date, *, latest_ingest_run: IngestRun | None = None
) -> DailyAggregate:
...
def persist_parsed_export(
    parsed: ParsedExport, *, parser_key: str, uploaded_by
) -> IngestSummary:
...
def ingest_export(
    uploaded_file: BinaryIO, *, parser_key: str, uploaded_by
) -> IngestSummary:
    ...
    return persist_parsed_export(parsed, parser_key=parser_key, uploaded_by=uploaded_by)
